---
title: "Pipeline of fig data analysis (RADseq data)"
---

*This is a Quarto website.
To learn more about Quarto websites visit <https://quarto.org/docs/websites>.*

## 0) Notes to begin with
...
`sbatch my_script.sh` (if your HPC system is using the slurm workload manager).
To check status of your job, run (for example):
`squeue --user=yourname --long`
...
-	To ensure your scripts are executable, change the permissions by running:
`chmod a+x script.sh`
...
To install fastqc, you can run the [fastqc_intall_linux.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/fastqc_intall_linux.sh") script, *or* run the following commands:
...
You can now run fastqc in your raw data by running the [fastqc.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/fastqc.sh") script *or* by running: 
...
You can then download the .html files and open for inspection. 
To download the files to your local machine, you can use (for example):
...
Since each sample has reads in multiple runs, I merge the raw files to a single file for each sample. To do this, I need to match all the relevant radtags to the right sample based on the key file provided by SNPSaurus (you can have a look at the key file here: [fig1617_key.txt]("/Users/u2095750/Documents/GitHub/website/docs/data files/fig1617_key.txt"))
Due to the relative complexity of the matching (radtags are shared across runs) I will do this manually to ensure there are no mistakes, so there is no shortcut or for-loop possible for this step. 
You can decide to exclude some raw reads file if the quality is bad. 

First, I create a folder for the coming merged files:

`mkdir raw_data_per_sample `

Then I run the script merging_data.sh (you can find it here:[merging_data.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/merging_data.sh")) which replicates this type of command for each sample:

`cat ./raw_data/raw_file_1.fastq.gz ./raw_data/raw_file_2.fastq.gz > ./raw_data_per_sample/sample_name.gz`

After this step, you might want to free space by removing the raw reads. 
**Be careful and make sure you do not need them anymore, as uploading again will be a big waste of time!**
...
If you need to install bowtie2, you need to run the bowtie2_install_linux.sh script ([bowtie2_install_linux.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/bowtie2_install_linux.sh")) *or* the bowtie2_install_mac.sh script ([bowtie2_install_mac.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/bowtie2_install_mac.sh")) script *or* run the following commands:
...
Download the reference genome (one file for whole genome and one file for cds regions only) by running reference_download.sh script ([reference_download.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/reference_download.sh")) *or* by running:
...
Run reference_index.sh ([reference_index.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/reference_index.sh")) *or* reference_index_mac.sh ([reference_index_mac.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/reference_index_mac.sh")) *or* run the following commands: 
...
Use the alignment_wg.sh ([alignment_wg.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/alignment_wg.sh")) and alignment_cds.sh ([alignment_cds.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/alignment_cds.sh")) scripts *or* run the following commands: 
...
**! Do NOT delete the slurm-out files from these jobs!
Alternatively, save the terminal output: this gives you information on the alignment details. **

We will also need to generate the .bam files and index them. 

### 5.5) Install samtools

First we need to install samtools. Run the samtools_install.sh script ([samtools_install.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/samtools_install.sh")) *or* run:
...
Make .bam files and index them by running the samtools.sh ([samtools.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/samtools.sh")) script, *or* samtools_mac.sh ([samtools_mac.sh]("/Users/u2095750/Documents/GitHub/website/docs/script files/samtools_mac.sh")) (if samtools is preinstalled in your local machine in any path) *or* the following commands:
...
i) whole genome alignments:
[wg_3_populations.txt]("/Users/u2095750/Documents/GitHub/website/docs/data files/wg_3_populations.txt")
[wg_4_populations.txt]("/Users/u2095750/Documents/GitHub/website/docs/data files/wg_4_populations.txt")
[wg_3_species.txt]("/Users/u2095750/Documents/GitHub/website/docs/data files/wg_3_species.txt")
[wg_4_species.txt]("/Users/u2095750/Documents/GitHub/website/docs/data files/wg_4_species.txt")

ii) cds alignments:
[cds_3_populations.txt]("/Users/u2095750/Documents/GitHub/website/docs/data files/cds_3_populations.txt")
[cds_4_populations.txt]("/Users/u2095750/Documents/GitHub/website/docs/data files/cds_4_populations.txt")
[cds_3_species.txt]("/Users/u2095750/Documents/GitHub/website/docs/data files/cds_3_species.txt")
[cds_4_species.txt]("/Users/u2095750/Documents/GitHub/website/docs/data files/cds_4_species.txt")
